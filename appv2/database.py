...
class Database:
    ...
    def save_system_metrics(self, metrics):
        """Salva as métricas do sistema no banco de dados
        
        Args:
            metrics (dict): Dicionário contendo as métricas do sistema com a seguinte estrutura:
                {
        ...
        try:
            ...
            ''', (
                ...
                metrics['process']['connections'],
                metrics['process']['open_files']
            ))
            
            self.conn.commit()
        except Exception as e:
            self.conn.rollback()
            raise Exception(f"Erro ao salvar métricas do sistema: {e}")            
    ...
    def save_pod_ports(self, pod_name, namespace, ports_info):
        """Salva informações das portas do pod no banco de dados"""
        try:
            cursor = self.conn.cursor()
            
            # Limpa registros antigos deste pod
            cursor.execute('''
                DELETE FROM pod_ports 
                WHERE pod_name = ? AND namespace = ?
            ''', (pod_name, namespace))
            ...
            self.conn.rollback()
            raise Exception(f"Erro ao salvar informações das portas: {e}")