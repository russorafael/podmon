...
class PodMonitor:
    ...
    def monitor_self(self):
        """Monitora o próprio sistema PodMon"""
        try:
            # Monitorar CPU
            ...
            if cpu_percent > 80:
                self.send_alert(
                    'Sistema PodMon - Alta utilização de CPU',
                    f'Utilização de CPU em {cpu_percent}%',
                    level='warning'
                )
                
            if memory_percent > 80:
                self.send_alert(
                    'Sistema PodMon - Alta utilização de Memória',
                    f'Utilização de memória em {memory_percent}%',
                    level='warning'
                )
                
            if disk_percent > 80:
                self.send_alert(
                    'Sistema PodMon - Disco próximo do limite',
                    f'Utilização de disco em {disk_percent}%',
                    level='warning'
                )
            ...
            logger.error(f"Erro ao monitorar sistema PodMon: {e}")
            return None
    ...
    def get_pod_disk_usage(self, pod):
        try:
            ...
            try:
                ...
                # Removido o parâmetro timeout_seconds que estava causando o erro
                resp = stream(
                ...
                # O timeout agora é controlado pelo run_forever
...
def get_system_metrics():
    """Obtém métricas do próprio sistema PodMon"""
    try:
        metrics = monitor.monitor_self()
        if metrics:
            return jsonify(metrics)
        return jsonify({'error': 'Não foi possível obter métricas do sistema'}), 500
    except Exception as e:
...
def background_monitor():
    ...
    while True:
        try:
            # Monitora pods e nodes
            monitor.get_pods_info()
            monitor.get_node_stats()
            
            # Monitora o próprio sistema
