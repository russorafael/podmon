...
// Configuração global
const config = {
    refreshInterval: 30000, // Alterar de 600000 para 30000
...
async function loadDashboardData() {
    showLoading(true);
    try {
        const [pods, nodes, history, systemMetrics] = await Promise.all([
            api.getPods(),
            api.getNodes(),
            api.getHistory(),
        ...
        updateSystemMonitorChart(systemMetrics);
    } catch (error) {
...
function updateSystemMonitorChart(metrics) {
    const ctx = document.getElementById('systemMonitorChart')?.getContext('2d');
    if (!ctx) return;

    const data = {
        labels: [], // Será preenchido com timestamps
        datasets: [
            {
                label: 'CPU Usage',
                data: [],
    ...
    if (metrics) {
        ...
        data.datasets[0].data.push(metrics.cpu_percent);
        data.datasets[1].data.push(metrics.memory_percent);
        data.datasets[2].data.push(metrics.disk_percent);
    }

    // Criar ou atualizar o gráfico
    if (systemMetricsChart) {
        systemMetricsChart.data = data;
